...
#use this create_texture to import image from AWS URL
def create_texture(filename, modelpath):
    name = filename
    texture = bpy.data.textures.new(name, type='IMAGE')

    tmp_filename = "/tmp/"+name+
    request.urlretrieve("https://blueprint-dev.s3.amazonaws.com/uploads/textures/"+name, tmp_filename)
    image = bpy.data.images.load(tmp_filename) #load_image(filename, modelpath)
    has_data = False
    ...
    return texture
# will this overload memory? '^ above' I keep creating new tmp files... 
#maybe you just start with a clean slate everytime so you don't have to worry about overloading memory each time... i.e. the startup file is clean

#This is the defaut three.js import
'''
def create_texture(filename, modelpath):
    name = filename
    texture = bpy.data.textures.new(name, type='IMAGE')
...
def create_mesh_object(name, vertices, materials, face_data, flipYZ, recalculate_normals):
    ...
    #-- Blender API gotcha: should make a tesselation after loading geometry
    ...
    if face_data["hasVertexColors"]:
        ...
        me.tessface_vertex_colors.new("vertex_color_layer_0")

        #-- Blender API gotcha: should re-tesselate after declaring color channel
        me.update(calc_tessface=True)
        #--
        ...
        for fi in range(len(faces)):
            ...
            if vertexColors[fi]:
                ...
                face_colors = me.tessface_vertex_colors[0].data[fi]
    ...
    if face_data["hasVertexUVs"]:
        ...
        #-- Blender API gotcha: should re-tesselate after declaring UV channels
        ...
        for li, layer in enumerate(vertexUVs):
            ...
            for fi in range(len(faces)):
                ...
                if layer[fi]:
                    ...
                    uv_face = me.tessface_uv_textures[li].data[fi]
    ...
    if face_data["hasMaterials"]:
        ...
        #-- Blender API gotcha: should re-tesselate after declaring materials
        me.update(calc_tessface=True)
        #--
    ...
    #-- Blender API gotcha: should call update to put tessfaces back into a mesh after all of our modifications
    me.update()
    #
